...
sudo iptables -t mangle -A PREROUTING -i eth0 -d 224.0.0.0/4 -j TTL --ttl-set 4
sudo iptables -t mangle -A PREROUTING -i bat0 -d 224.0.0.0/4 -j TTL --ttl-set 4


ip route add ${HOME_NET} dev eth0
...
# in fritzbox jetzt noch static route eintragen: um von home subnet zu den 10.10.x.0/24 netzen zu kommen
